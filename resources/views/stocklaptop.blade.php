            ...
            <div class="row">
                <div class="col-12">
                    <div class="card rounded border-0 shadow mx-1 my-4 p-3">
                        <a class="btn mx-1 my-1 outline-primary-spatu" href="/tambahdatastock" style="width: 10%">Tambah
                            +</a>
                        <table class="table" id="table_id">
                            <thead class="color-primary-spatu">
                                <tr>
                                    <th scope="col">No</th>
                                    <th scope="col">Foto</th>{{-- file --}}
                                    <th scope="col">Merk</th>{{-- text --}}
                                    <th scope="col">Proseccor</th>{{-- checkbox --}}
                                    <th scope="col">Ram</th>{{-- radiobutton --}}
                                    <th scope="col">VGA</th>{{-- Dropdown --}}
                                    <th scope="col">Harga jual</th>{{-- Dropdown --}}
                                    <th scope="col">stock</th>{{-- number --}}
                            ...
                            <tbody>
                                ...
                                @foreach ($data as $row)
                                    <tr>
                                        <th scope="row">{{ $no++ }}</th>
                                        <td><img src="{{ asset('fotopegawai/' . $row->foto) }}" alt=""
                                                width="100px">
                                        </td>
                                        <td>{{ $row->merk }}</td>
                                        <td>{{ $row->processor ? $row->processor:'-';  }}</td>
                                        <td>{{ $row->ram }}</td>
                                        <td>{{ $row->VGA }}</td>
                                        <td>{{"Rp." . number_format($row->harga_jual,0,',','.')}}</td>
                                        <td>{{ $row->stock }}</td>
                                        <td>
                                            <a href="/editdatastockform/{{ $row->id }}"><button type="button"
                                                    class="btn btn-success"><i
                                                        class="fa-solid fa-pen-to-square"></i></button></a>
                                            <a href="#" class="btn btn-danger delete"
                                                data-id="{{ $row->id }}" data-merk="{{ $row->merk }}"><i
